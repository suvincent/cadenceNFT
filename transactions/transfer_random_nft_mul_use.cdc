...
transaction (nft_id: UInt64,receiver: Address, nums: [Int], experiences:[String]){
    ...
    prepare(acct: AuthAccount) {
        ...
        // useNFT
        let receiverCollectionRef = recipient.getCapability(EverSinceNFT.CollectionPublicPath)
            .borrow<&{EverSinceNFT.EverSinceNFTCollectionPublic}>()
            ?? panic("Could not borrow a reference to the receiver's collection")
        let nft = receiverCollectionRef.borrowEverSinceNFT(id: nft_id)!
